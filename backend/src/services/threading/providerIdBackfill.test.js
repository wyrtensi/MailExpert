...
describe('runProviderIdBackfill', () => {
  it('fills ids, saves the cursor per folder and marks the run finished', async () => {
    ...
    expect(result).toEqual({ outcome: 'done', processed: 3, total: 3, failedFolders: [], skippedFolders: [] });
  ...
  it('passes rows the server no longer has, so a second run finds nothing to do', async () => {
    ...
    expect(second).toEqual({ outcome: 'done', processed: 0, total: 0, failedFolders: [], skippedFolders: [] });
  ...
  it('resumes after the saved cursor', async () => {
    ...
    expect(await run(db, client)).toEqual({ outcome: 'done', processed: 1, total: 1, failedFolders: [], skippedFolders: [] });
  ...
  it('skips a folder whose server UIDVALIDITY no longer matches the cached rows and reports the run incomplete', async () => {
    const db = fakeDb({ folders: FOLDERS, messages: [row('INBOX', 1), row('[Gmail]/Sent Mail', 2)] });
    const client = fakeClient({ INBOX: [gm(1)], '[Gmail]/Sent Mail': [gm(2)] });
    client.getMailboxLock.mockImplementation(async (path) => {
      client.mailbox = { path, uidValidity: path === 'INBOX' ? 9n : 7n };
      return { release: vi.fn() };
    });

    const result = await run(db, client);

    expect(result).toEqual({ outcome: 'incomplete', processed: 1, total: 2, failedFolders: [], skippedFolders: ['INBOX'] });
    expect(client.fetch.mock.calls.map(c => c[0])).toEqual(['2']);
    expect(db.db.updates).toEqual([{ folder: '[Gmail]/Sent Mail', uids: [2] }]);
    expect(db.db.state?.cursors?.INBOX).toBeUndefined();
    expect(db.db.finished).toBe(0);
  });

  it('fills a folder whose stored UIDVALIDITY is unknown', async () => {
    const db = fakeDb({ folders: [{ path: 'INBOX', uid_validity: null }], messages: [row('INBOX', 1)] });
    const client = fakeClient({ INBOX: [gm(1)] }, { uidValidity: 9n });

    const result = await run(db, client);

    expect(result).toEqual({ outcome: 'done', processed: 1, total: 1, failedFolders: [], skippedFolders: [] });
    expect(db.db.messages[0].provider_message_id).toBe('1800000000000000001');
    expect(db.db.state.cursors.INBOX).toEqual({ lastUid: 1, uidValidity: null });
    expect(db.db.finished).toBe(1);
  });

  it('records a folder the server refuses and still processes the folders after it', async () => {
    const db = fakeDb({ folders: FOLDERS, messages: [row('INBOX', 1), row('[Gmail]/Sent Mail', 2)] });
    const client = fakeClient({ INBOX: [gm(1)], '[Gmail]/Sent Mail': [gm(2)] });
    const releases = [];
    client.getMailboxLock.mockImplementation(async (path) => {
      if (path === 'INBOX') {
        throw Object.assign(new Error('Command failed'), { responseStatus: 'NO', responseText: 'Mailbox does not exist' });
      }
    ...
    expect(result).toEqual({
      outcome: 'incomplete', processed: 1, total: 2,
      failedFolders: [{ path: 'INBOX', error: 'Mailbox does not exist' }], skippedFolders: [],
    });
    expect(db.db.messages.map(m => m.provider_message_id)).toEqual([null, '1800000000000000002']);
    expect(db.db.state.cursors.INBOX).toBeUndefined();
  ...
  it('keeps the cursor of a folder whose FETCH is rejected with BAD and moves on', async () => {
    const messages = Array.from({ length: PROVIDER_ID_BATCH_SIZE + 1 }, (_, i) => row('INBOX', i + 1));
    const db = fakeDb({ folders: FOLDERS, messages: [...messages, row('[Gmail]/Sent Mail', 3)] });
    const client = fakeClient({ INBOX: messages.map(m => gm(m.uid)), '[Gmail]/Sent Mail': [gm(3)] });
    const realFetch = client.fetch.getMockImplementation();
    client.fetch
      .mockImplementationOnce(realFetch)
      .mockImplementationOnce(() => (async function* () {
        yield* [];
        throw Object.assign(new Error('Command failed'), { responseStatus: 'BAD' });
      })());

    const result = await run(db, client);

    expect(result.outcome).toBe('incomplete');
    expect(result.failedFolders).toEqual([{ path: 'INBOX', error: 'Command failed' }]);
    expect(db.db.state.cursors.INBOX).toEqual({ lastUid: PROVIDER_ID_BATCH_SIZE, uidValidity: '7' });
    expect(db.db.state.cursors['[Gmail]/Sent Mail']).toEqual({ lastUid: 3, uidValidity: '7' });
    expect(db.db.finished).toBe(0);
  });

  it('passes the pending mark read at the start to the finished mark', async () => {
    const pendingSince = new Date('2026-09-17T10:00:00.123Z');
    const db = fakeDb({ folders: FOLDERS, messages: [row('INBOX', 1)], state: { cursors: {}, pending_since: pendingSince } });
    const client = fakeClient({ INBOX: [gm(1)] });

    await run(db, client);

    expect(db.db.finishedWith).toEqual([pendingSince]);
  });

  it('pauses once per batch', async () => {
    const db = fakeDb({ folders: FOLDERS, messages: [row('INBOX', 1), row('[Gmail]/Sent Mail', 2)] });
    const client = fakeClient({ INBOX: [gm(1)], '[Gmail]/Sent Mail': [gm(2)] });
    const pause = vi.fn(async () => {});

    await run(db, client, { pause });
  ...
  it('stops before the next batch when asked, without marking the run finished', async () => {
    ...
    expect(result).toEqual({ outcome: 'stopped', processed: 0, total: 1, failedFolders: [], skippedFolders: [] });
  ...
  it('releases the mailbox lock and keeps earlier cursors when a fetch fails', async () => {
    ...
    // No IMAP response status: a transport failure ends the run instead of failing one folder.
