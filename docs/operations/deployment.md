...
`<MAIL_HOST>` — будущий почтовый узел (PR 9).
...
- S3-совместимый бакет **у другого провайдера, чем сам сервер** — для бэкапов restic.
...
## 7. Переезд почтового узла (позже, PR 9)

Пока не реализовано, но раскладка уже рассчитана на это: MailExpert хранит только `<MAIL_HOST>`,
IP узла не хранится нигде — ни в базе, ни в `.env`. Перенос узла — это:

- собственный бэкап и восстановление mailcow (`helper-scripts/backup_and_restore.sh`);
- на новом сервере — тот же `<MAIL_HOST>`, новый PTR-запись и открытый порт 25;
- коннекторы Microsoft EOP переводятся на новый IP (или сразу на имя/сертификат, если ещё не
  были);
- MailExpert при этом менять нечего: имя хоста то же, IMAP-соединения переподключаются сами.

Подробности появятся вместе с самим PR 9.
