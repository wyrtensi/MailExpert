...
// inactive; an inactive tab can still be opened and says why instead of showing its form. With no
// `active` yet (the statuses are loading) no tab is chosen and the panel says it is loading.
export default function AddAccountTabs({ options, active, onSelect, renderForm }) {
  ...
  return (
    <div>
      ...
      <div id="add-account-panel" role="tabpanel" aria-labelledby={current ? `add-account-tab-${current.kind}` : undefined}>
        {current && <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginBottom: 14 }}>{t(current.descriptionKey)}</div>}
        {current?.enabled
