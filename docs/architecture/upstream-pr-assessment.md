...
- Пробуждение ожидающих в пуле после неудачного подключения: upstream сознательно отказался (каждое ожидание делало бы свой логин к серверу, который только что отказал), мы тоже.
...
- Сообщения о `poolExhausted` в маршрутах bulk-read и флагов: upstream оставил их на потом, у нас так же.
