...
// GET /thread/:threadId (DISTINCT ON (m.account_id, COALESCE(m.message_id, m.id::text)))
// already dedupe it. A row with no message_id can't be matched to any duplicate, so it counts
// as its own letter, as it does in GET /thread/:threadId.
